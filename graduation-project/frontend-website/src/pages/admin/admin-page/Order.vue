...
<template lang="">
  <CommonAdmin title="Đơn hàng" actionNew="false" class="relative">
    <template v-slot:search>
      <form class="flex w-[500px] gap-2 items-center justify-center">
        <Input
          type="text"
          placeholder="Tìm kiếm..."
          name="search"
          id="search"
          classChild=" py-[6px] rounded-sm max-w-[300px] max-h-[40px]"
        ...
        <Button
          ...
          id="search"
          @click="onSearch"
          className="bg-[#80bb35] text-[white] hover:bg-[#bde18e] hover:text-white w-[150px]"
        />
      </form>
    </template>
        ...
        <tr v-for="(item, index) in api.data" :key="item.orderId">
          ...
          <td class="flex items-center justify-around gap-2">
            <router-link :to="{ name: 'OrderDetail', params: { orderId: item.orderId } }">
              <button
                className="min-w-[60px] px-2 text-sm bg-green-600 hover:bg-green-600 block text-center m-0 text-white"
                name="edit"
    ...
    <template v-slot:page>
      <ul class="flex items-center justify-end gap-1">
        <li class="bg-[#80bb35] text-[white] hover:opacity-80" @click="onPreviousPage">Sau</li>
        <!-- <li
        ...
        <li class="bg-[#80bb35] text-[white] hover:opacity-80" @click="onNextPage">Tiếp</li>
      </ul>
    </template>
  </CommonAdmin>
</template>
